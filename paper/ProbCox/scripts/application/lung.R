...
write.csv(lung, './data/application/lung.csv')

m = coxph(Surv(time, status) ~., data=lung)
summary(m)

x = paste(unname(m$coefficients), collapse="; ")
write(x, file='./out/application/lung/R_theta.txt', ncolumns = 1, append = FALSE, sep = ";")

x = paste(sqrt(diag(m$var)), collapse="; ")
write(x, file='./out/application/lung/R_se.txt', ncolumns = 1, append = FALSE, sep = ";")

x = paste(unname(m$concordance['concordance']), collapse=" ")
write(x, file='./out/application/lung//R_concordance.txt', ncolumns = 1, append = FALSE, sep = ";")
