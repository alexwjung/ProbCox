...
write.csv(nafld1, './data/application/nafld1.csv')

m = coxph(Surv(futime, status) ~., data=nafld1)
summary(m)

x = paste(unname(m$coefficients), collapse="; ")
write(x, file='./out/application/nafld1/R_theta.txt', ncolumns = 1, append = FALSE, sep = ";")

x = paste(sqrt(diag(m$var)), collapse="; ")
write(x, file='./out/application/nafld1/R_se.txt', ncolumns = 1, append = FALSE, sep = ";")

x = paste(unname(m$concordance['concordance']), collapse=" ")
write(x, file='./out/application/nafld1//R_concordance.txt', ncolumns = 1, append = FALSE, sep = ";")
