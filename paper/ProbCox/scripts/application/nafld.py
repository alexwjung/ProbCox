'''
Analysing the nafld data from the R - survival package
'''
...
nafld = pd.read_csv('./data/application/nafld1.csv', sep=',')
...
np.savetxt('./out/application/nafld1/concordance.txt', ci)
np.savetxt('./out/application/nafld1/se.txt', torch.diag(pyro.get_param_store()['AutoMultivariateNormal.scale_tril']).detach().numpy())

with open('./out/application/nafld1/theta_lower.txt', 'a') as write_out:
    write_out.write(''.join([str(ii) + '; ' for ii in out['theta'][0].detach().numpy()[:, 0].tolist()]))
    write_out.write('\n')
with open('./out/application/nafld1/theta.txt', 'a') as write_out:
    write_out.write(''.join([str(ii) + '; ' for ii in out['theta'][1].detach().numpy()[:, 0].tolist()]))
    write_out.write('\n')
with open('./out/application/nafld1/theta_upper.txt', 'w') as write_out:
