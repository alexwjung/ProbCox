...
for ii in (range((iter_))):
    ...
    if ii%25==0:
        ...
        with torch.no_grad():
            ...
            print(pcox.CoxPartialLikelihood(pred=pred, sampling_proportion=None).log_prob(surv=surv).detach().numpy())


            fig, ax = plt.subplots(1, 1, figsize=(4*cm, 4*cm), dpi=300)
            ax.scatter(theta, theta_est, s=2)
            ax.set_xlim([-1.5, 1.5])
            ax.set_ylim([-1.5, 1.5])
            ax.plot(ax.get_xlim(), ax.get_ylim(), ls=':', color='black', linewidth=0.5)
            plt.show()
            plt.close()
