'''
High Dimensional Case Simulation - prior elicitation:


individuals:  300
covaraites:   250 binary (0.2), 250 Normal(0, 1)
theta:        ~ N(0, 1^2)
runs:         50 - Seed = 1, 2, ..., 50
...
theta = np.random.normal(0, 1, 20)[:, None]

theta = np.concatenate((theta[:10], np.zeros((240, 1)), theta[10:], np.zeros((240, 1))))
...
if run_id != 0:
    # batch model:
    pyro.clear_param_store()
    out = evaluate(run_suffix='rank10_ST1_0.001', rank=10, batchsize=256, iter_=10000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])
    
    pyro.clear_param_store()
    out = evaluate(run_suffix='rank100_ST1_0.001', rank=100, batchsize=256, iter_=10000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])

# =======================================================================================================================

def predictor(data):
    theta =  pyro.sample("theta", dist.StudentT(2, loc=0, scale=0.01).expand([data[1].shape[1], 1])).type(dtype)
    pred = torch.mm(data[1], theta)
    return(pred)
...
if run_id != 0:
    # batch model:
    pyro.clear_param_store()
    out = evaluate(run_suffix='rank10_ST2_0.01', rank=10, batchsize=256, iter_=10000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])

# =======================================================================================================================


def predictor(data):
    theta =  pyro.sample("theta", dist.StudentT(4, loc=0, scale=0.1).expand([data[1].shape[1], 1])).type(dtype)
    pred = torch.mm(data[1], theta)
    return(pred)
...
if run_id != 0:
    # batch model:
    pyro.clear_param_store()
    out = evaluate(run_suffix='rank10_ST4_0.1', rank=10, batchsize=256, iter_=10000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])

# =======================================================================================================================

def predictor(data):
    theta =  pyro.sample("theta", dist.Laplace(loc=0, scale=0.5).expand([data[1].shape[1], 1])).type(dtype)
    pred = torch.mm(data[1], theta)
    return(pred)
...
if run_id != 0:
    # batch model:
    pyro.clear_param_store()
    out = evaluate(run_suffix='rank10_L0.5', rank=10, batchsize=256, iter_=10000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])
...
if run_id != 0:
    # batch model:
    pyro.clear_param_store()
    out = evaluate(run_suffix='rank10_L0.25', rank=10, batchsize=256, iter_=15000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])
...
def predictor(data):
    theta =  pyro.sample("theta", dist.Laplace(loc=0, scale=0.1).expand([data[1].shape[1], 1])).type(dtype)
    pred = torch.mm(data[1], theta)
    return(pred)
...
if run_id != 0:
    # batch model:
    pyro.clear_param_store()
    out = evaluate(run_suffix='rank10_L0.1', rank=10, batchsize=256, iter_=15000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])

# =======================================================================================================================

#for i in 1; do bsub -env "VAR1=$i"  -n 4 -M 16000 -R "rusage[mem=8000]" './highdimensional_case4.sh'; sleep 1; done
#for i in {1..50}; do bsub -env "VAR1=$i" -o /dev/null -e /dev/null -n 2 -M 16000 -R "rusage[mem=6000]" './highdimensional_case4.sh'; sleep 1; done
