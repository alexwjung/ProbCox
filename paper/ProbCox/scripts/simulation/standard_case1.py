...
if run_id != 0:
    ...
    write(x, file = paste(ROOT_DIR, '/out/sim_sc1/R_theta.txt', sep=''), ncolumns = 1, append = TRUE, sep = ";")

    x = paste(sim_name, paste(sqrt(diag(m$var)), collapse="; "), sep='; ')
    write(x, file = paste(ROOT_DIR, '/out/sim_sc1/R_se.txt', sep=''), ncolumns = 1, append = TRUE, sep = ";")
    '''
...
# cluster submission
#for i in {101..200}; do bsub -env "VAR1=$i" -o /dev/null -e /dev/null -n 2 -M 2000 -R "rusage[mem=500]" './standard_case1.sh'; sleep 30; done