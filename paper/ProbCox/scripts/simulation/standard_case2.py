...
if run_id != 0:
    ...
    write(x, file = paste(ROOT_DIR, '/out/simulation/sim_sc2/R_theta.txt', sep=''), ncolumns = 1, append = TRUE, sep = ";")

    x = paste(sim_name, paste(sqrt(diag(m$var)), collapse="; "), sep='; ')
    write(x, file = paste(ROOT_DIR, '/out/simulation/sim_sc2/R_se.txt', sep=''), ncolumns = 1, append = TRUE, sep = ";")
    '''
