...
if run_id != 0:
    ...
    write(x, file = paste(ROOT_DIR, '/out/sim_sc3/R_theta.txt', sep=''), ncolumns = 1, append = TRUE, sep = ";")

    x = paste(sim_name, paste(sqrt(diag(m$var)), collapse="; "), sep='; ')
    write(x, file = paste(ROOT_DIR, '/out/sim_sc3/R_se.txt', sep=''), ncolumns = 1, append = TRUE, sep = ";")
    '''
...
# cluster submission
#for i in {1..100}; do bsub -env "VAR1=$i" -o /dev/null -e /dev/null -n 2 -M 2500 -R "rusage[mem=1000]" './standard_case3.sh'; done
