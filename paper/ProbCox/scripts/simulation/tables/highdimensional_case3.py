...
for suffix in ['rank10', 'rank30', 'rank50']:
...
, W, col_idx)
    ...
    pd.DataFrame(res[1490:1510, :]).to_csv('./out/simulation/tables/' + sim_name + '_ProbCox_main_' + suffix + '.csv')

    theta_est_lower = theta_bound[:, :3000]
    theta_est_upper = theta_bound[:, 3000:]
...
for suffix in ['R_lasso_theta', 'R_lasso_theta_1se', 'R_Alasso1_theta', 'R_Alasso1_theta_1se', 'R_Alasso2_theta', 'R_Alasso2_theta_1se']:
    ...
    res = pd.DataFrame(res[1490:1510,:])
    res.iloc[:, 4] = '-'
    res.iloc[:, 5] = '-'
