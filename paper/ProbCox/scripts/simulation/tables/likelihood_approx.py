...
MPE = pd.read_csv('./out/simulation/tables/likelihood_approx_MPE.csv').iloc[:, 1:]
MAPE = pd.read_csv('./out/simulation/tables/likelihood_approx_MAPE.csv').iloc[:, 1:]

for ii in range(MPE.shape[0]):
    for jj in range(1, MPE.shape[1]):
        MPE.iloc[ii, jj] = str(MPE.iloc[ii, jj]) + ' (' + str(MAPE.iloc[ii, jj]) + ')'
MPE.to_csv('./out/simulation/tables/likelihood_approx_combined.csv')
