'''
Likelihood Approximation Simulation:

- Evaluating the approximation to the likelihood through the proposed subsampling.
- Generate table from main paper

Settings:
- individuals: - I - 5000, 10000
...
pd.DataFrame(np.concatenate((np.asarray(sim_n)[:, None], res.astype(str)), axis=1)).to_csv('./out/tables/likelihood_approx_MPE.csv')
pd.DataFrame(np.concatenate((np.asarray(sim_n)[:, None], res2.astype(str)), axis=1)).to_csv('./out/tables/likelihood_approx_MAPE.csv')


# Adjustments
# =======================================================================================================================

MPE = pd.read_csv('./out/tables/likelihood_approx_MPE.csv').iloc[:, 1:]
MAPE = pd.read_csv('./out/tables/likelihood_approx_MAPE.csv').iloc[:, 1:]

for ii in range(MPE.shape[0]):
    for jj in range(1, MPE.shape[1]):
        MPE.iloc[ii, jj] = str(MPE.iloc[ii, jj]) + ' (' + str(MAPE.iloc[ii, jj]) + ')'
MPE.to_csv('./out/tables/likelihood_approx_combined.csv')      
