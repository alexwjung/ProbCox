...
write.csv(heart, './data/real/heart.csv')

m = coxph(Surv(start, stop, event) ~., data=heart)
summary(m)

x = paste(unname(m$coefficients), collapse="; ")
write(x, file='./output/real/heart/R_theta.txt', ncolumns = 1, append = FALSE, sep = ";")

x = paste(sqrt(diag(m$var)), collapse="; ")
write(x, file='./output/real/heart/R_se.txt', ncolumns = 1, append = FALSE, sep = ";")

x = paste(unname(m$concordance['concordance']), collapse=" ")
write(x, file='./output/real/heart//R_concordance.txt', ncolumns = 1, append = FALSE, sep = ";")
