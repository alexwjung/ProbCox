'''
Plot 1 - Log-likelihood approximation over training
'''
...
# Simulation Settings:
# -----------------------------------------------------------------------------------------------------------------------------
theta = np.random.uniform(-1.5, 1.5, (6, 1))
TVC = sim.TVC(theta=theta, P_binary=3, P_continuous=3, dtype=dtype)
TVC.make_lambda0(scale=0.75)
...
ax[0].plot(np.arange(5000), -np.asarray(LL_full)[:5000], color='#0b64e0', label='full')
...
plt.savefig('./output/simulation/figures/plot1.eps', bbox_inches='tight')
