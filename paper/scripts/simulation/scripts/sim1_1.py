...
#np.sum([torch.sum(TVC.sample()[0][:, -1]).numpy() for ii in tqdm.tqdm(range(1000))])
...
# batch model:
pyro.clear_param_store()
out = evaluate(run_suffix='full', batchsize=total_obs, iter_=3000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])

pyro.clear_param_store()
out = evaluate(run_suffix='1024', batchsize=1024, iter_=3000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])

pyro.clear_param_store()
out = evaluate(run_suffix='512', batchsize=512, iter_=3000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])

pyro.clear_param_store()
out = evaluate(run_suffix='256', batchsize=256, iter_=3000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])

pyro.clear_param_store()
out = evaluate(run_suffix='128', batchsize=128, iter_=3000, surv=surv, X=X, sampling_proportion=[total_obs, None, total_events, None])
...
write(x, file = paste(ROOT_DIR, '/output/simulation/sim1_1/R_theta.txt', sep=''), ncolumns = 1, append = TRUE, sep = ";")

x = paste(sim_name, paste(sqrt(diag(m$var)), collapse="; "), sep='; ')
write(x, file = paste(ROOT_DIR, '/output/simulation/sim1_1/R_se.txt', sep=''), ncolumns = 1, append = TRUE, sep = ";")
'''
