...
- Addition with large linear effect
...
for t in T:
    ...
    while cond:
        ...
        TVC = sim.TVC(theta=theta, P_binary=int(10), P_continuous=int(10), dtype=dtype)
...
pd.DataFrame(np.concatenate((np.asarray(sim_n)[:, None], res.astype(str)), axis=1)).to_csv('./output/simulation/tables/likelihood_approx_MPE_additonal2.csv')
pd.DataFrame(np.concatenate((np.asarray(sim_n)[:, None], res2.astype(str)), axis=1)).to_csv('./output/simulation/tables/likelihood_approx_MAPE_additonal2.csv')

# combine arrays

MPE = pd.read_csv('./output/simulation/tables/likelihood_approx_MPE_additonal2.csv').iloc[:, 1:]
MAPE = pd.read_csv('./output/simulation/tables/likelihood_approx_MAPE_additonal2.csv').iloc[:, 1:]

for ii in range(MPE.shape[0]):
    for jj in range(1, MPE.shape[1]):
        MPE.iloc[ii, jj] = str(MPE.iloc[ii, jj]) + ' (' + str(MAPE.iloc[ii, jj]) + ')'
MPE.to_csv('./output/simulation/tables/likelihood_approx_additonal2_combined.csv')      
