...
theta = np.asarray(pd.read_csv('./output/simulation/sim1_1/theta.txt', header=None))
...
res = np.zeros((6, 6))
res[:, 0] = theta[:, 0]
theta_est = pd.read_csv('./output/simulation/sim1_1/R_theta.txt', header=None, sep=';')
theta_est = theta_est.dropna(axis=0)
theta_est = theta_est.groupby(0).first().reset_index()

theta_se = pd.read_csv('./output/simulation/sim1_1/R_se.txt', header=None, sep=';')
...
res = np.round(res, 2)
pd.DataFrame(res)
pd.DataFrame(res).to_csv('./output/simulation/tables/sim1_1_' + 'R' + '.csv')
...
batchsize  = '128'
theta_est = pd.read_csv('./output/simulation/sim1_1/probcox' + str(batchsize) + '_theta.txt', header=None, sep=';')
theta_est = theta_est.dropna(axis=0)
theta_est = theta_est.groupby(0).first().reset_index()
theta_est = theta_est.iloc[:, :-1]

theta_est_lower = pd.read_csv('./output/simulation/sim1_1/probcox' + str(batchsize) + '_theta_lower.txt', header=None, sep=';')
...
theta_est_upper = pd.read_csv('./output/simulation/sim1_1/probcox' + str(batchsize) + '_theta_upper.txt', header=None, sep=';')
...
res = np.round(res, 2)
pd.DataFrame(res)
pd.DataFrame(res).to_csv('./output/simulation/tables/sim1_1_' + batchsize + '.csv')
