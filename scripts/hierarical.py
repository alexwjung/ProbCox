...
Original file is located at
    https://colab.research.google.com/drive/1f5sBbiiMKREWQjFbNWKDrtcRQGDHGafd
"""

#!pip3 install -i https://test.pypi.org/simple/ probcox-testing==0.0.2
...
theta = np.asarray([-0.5, 1.4, 0.3, -0.9, 1.1, -0.4])[:, None]
...
# Simulation - Normal Frailty per individual - N(0, 3)
# =======================================================================================================================
np.random.seed(34)
torch.manual_seed(234)
...
ii = 0
for __ in (range(I)):
    a, b = TVC.sample(frailty=np.random.normal(0, 3, (1,)))
    Z_ = torch.zeros((a.shape[0], I))
    Z_[:, ii] = 1
    surv = torch.cat((surv, a))
    X = torch.cat((X, b))
...
def predictor(data):
    ...
    theta_z =  pyro.sample("theta_z", dist.Normal(loc=0, scale=1).expand([1000, 1])).type(dtype)
    pred = torch.mm(data[1], theta)  + torch.mm(data[2], theta_z)
    return(pred)
...
def evaluate(surv, X, batchsize, sampling_proportion, iter_, predictor=predictor):
    ...
    while run:
        ...
        for ii in tqdm.tqdm(range((iter_))):
            idx = np.unique(np.concatenate((np.random.choice(np.where(surv[:, -1]==1)[0], 2, replace=False), np.random.choice(range(surv.shape[0]), batchsize-2, replace=False)))) # random sample of data - force at least two events (no evaluation otherwise)
            data=[surv[idx], X[idx], Z[idx]] # subsampled data
            loss.append(m.infer(data=data))
            # divergence check
            if loss[-1] != loss[-1]:
                eta = eta * 0.1
...
csr = torch._sparse_csr_tensor(crow_indices, col_indices, values, dtype=torch.float)
csr

torch.matmul(csr, torch.normal(0, 1, (4, 1)))
